...
async def process_about(call: CallbackQuery, session_without_commit: AsyncSession):
    ...
    await bot.send_invoice(
        ...
        title=f'Оплата 👉 {price}BYN',
        description=f'Пожалуйста, завершите оплату в размере {price}BYN, чтобы открыть доступ к выбранному товару.',
        payload=f"{user_info.id}_{product_id}",
        provider_token=settings.PROVIDER_TOKEN,
        currency='BYN',
        prices=[LabeledPrice(label="Товар", amount=int(float(price) * 100))],
        ...
        try:
            ...
            await bot.send_message(
                ...
                text=(
                    ...
                    f"за <b>{product_data.price} BYN</b>."
                )
            )
